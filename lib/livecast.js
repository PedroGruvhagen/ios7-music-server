...
function startFfmpeg(pipePath) {
  ...
  var args = [
    ...
    '-ac', '2',
    '-i', pipePath,
    // shairport-sync's ignore_volume_control (see the .conf snippet in the
    // README/lore) deliberately ignores the AirPlay sender's own volume
    // slider so loudness is controlled only by the receiving device -- but
    // that also means whatever level the sender is internally driving at
    // reaches here completely unprotected. A limiter here is what keeps that
    // from turning into clipped/distorted noise the moment someone turns the
    // source app's own volume up, without having to re-introduce a volume
    // ceiling that would make output level depend on the sender again.
